        ...
        while stack.size() > 0:
            ...
            if len(unexplored_directions) > 0:
                ...
                for i in range (len(returned_path)):
                    if end_looping == False:
                        keep_track = len(returned_path) - 1
                        print('room', returned_path[i])
                        # print('next_room', returned_path[i + 1])
                        for key in self.rooms[returned_path[i]]:
                            print('key', key)
                            if self.rooms[returned_path[i]][key] == returned_path[i + 1]:
                                directions.append(key)
                                if len(directions) == keep_track:
                                    end_looping = True
